...
function draw() 
{
    ...
    if(kb.pressing('d'))
    {
        if(myAnimation.isColliding(scareCrow))
        {
            ...
            if (scareCrow != null) {
                if (dist(myAnimation.getCurrentAnimation().position.x, myAnimation.getCurrentAnimation().position.y, scareCrow.position.x, scareCrow.position.y) < 200) {
                    createParticles(scareCrow.position.x, scareCrow.position.y);
                    scareHealth -= 1;
                    if(scareHealth <= 0)
                    {
                        scareCrow.remove();
        ...
        else if(myAnimation.isColliding(sirKnight))
        {
            myAnimation.drawAnimation('idle');
            myAnimation.updatePosition('idle');
        }  
            ...
            myAnimation.drawAnimation('attack');
            if (sirKnight != null) {
                if (dist(myAnimation.getCurrentAnimation().position.x, myAnimation.getCurrentAnimation().position.y, sirKnight.position.x, sirKnight.position.y) < 200) {
                    createParticles(sirKnight.position.x, sirKnight.position.y);
                    knightHealth -= 1;
                    if(knightHealth <= 0)
                    {
                        sirKnight.remove();
        ...
        else if(myAnimation.isColliding(Slade))
        {
            myAnimation.drawAnimation('idle');
            myAnimation.updatePosition('idle');
        }  
            ...
            myAnimation.drawAnimation('attack');
            if (Slade != null) {
                if (dist(myAnimation.getCurrentAnimation().position.x, myAnimation.getCurrentAnimation().position.y, Slade.position.x, Slade.position.y) < 200) {
                    createParticles(Slade.position.x, Slade.position.y);
                    sladeHealth -= 1;
                    if(sladeHealth <= 0)
                    {
                        Slade.remove();
